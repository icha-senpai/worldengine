...
class ProgressionService
{
    ...
    public function snapshotFor(ConnectedRealmsPlayer $player, int $totalExperience, int $inventoryQuantity): array
    {
        ...
        return [
            ...
            'achievements' => $this->applyAchievementClaims($this->contentAchievements([
                ...
                $this->achievement('apprentice_artisan', 'Workbench Initiate', 'Reach level 5 in any crafting or processing profession.', max($highestCategoryLevel('Crafting'), $highestCategoryLevel('Processing')) >= 5, 'Crafting'),
                $this->achievement('master_artisan', 'Workshop Master', 'Reach level 50 in any crafting or processing profession.', max($highestCategoryLevel('Crafting'), $highestCategoryLevel('Processing')) >= 50, 'Crafting'),
                $this->achievement('reliable_hand', 'Reliable Hand', 'Turn in 5 board commissions.', $jobCount >= 5, 'Jobs'),
                $this->achievement('guild_worker', 'Board Regular', 'Turn in 25 board commissions.', $jobCount >= 25, 'Jobs'),
                $this->achievement('contract_legend', 'Commission Legend', 'Turn in 100 board commissions.', $jobCount >= 100, 'Jobs'),
    ...
    private function achievement(string $key, string $label, string $description, bool $unlocked, string $category): array
    {
        return [
            ...
            'reward' => $this->achievementReward($key, $label),
        ];
    }
     ...
     */
    private function achievementReward(string $key, string $label): array
    {
        $level = (int) (preg_match('/_(100|80|65|50|40|30|20|10|5|1)$/', $key, $matches) === 1 ? $matches[1] : 0);

        return [
            'title' => $this->achievementTitle($key, $label, $level),
            'gold' => match (true) {
                $level >= 100 => 1000,
                $level >= 80 => 650,
                $level >= 65 => 500,
