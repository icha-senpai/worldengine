    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span v-if="item.parent" class="text-muted-3 text-xs font-mono hover:text-muted-2 transition-colors">
                            <Link :href="route('pipeline.show', item.parent.id)">{{ item.parent.title }}</Link>
                        </span>
                        <span v-if="item.parent" class="text-muted-3 text-xs font-mono">/</span>
        ...
        <div class="space-y-5">
            ...
            <div v-if="item.content" class="panel">
                ...
                <div class="text-muted-2 text-sm leading-relaxed whitespace-pre-wrap">{{ item.content }}</div>
            </div>
            ...
            <div v-if="item.pipeline_type === 'scene'" class="grid grid-cols-2 gap-4">
                ...
                <div class="panel">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-start gap-2">
                            ...
                            <span v-if="item.pov_character" class="text-muted-2 text-xs">
                        ...
                        <div class="flex items-start gap-2">
                            ...
                            <span v-if="item.location" class="text-muted-2 text-xs">
                ...
                <div v-if="item.narrative_purpose" class="panel">
                    ...
                    <p class="text-muted-2 text-xs leading-relaxed whitespace-pre-wrap">{{ item.narrative_purpose }}</p>
                </div>
            ...
            <div v-if="item.pipeline_type === 'character_study' && (item.tracked_entity || item.arc_stage || item.arc_notes)" class="panel">
                ...
                <div class="space-y-2">
                    <div v-if="item.tracked_entity" class="flex items-start gap-2">
                        ...
                        <Link :href="route('entities.show', item.tracked_entity.id)" class="text-muted-2 text-xs hover:text-primary transition-colors">
                    ...
                    <div v-if="item.arc_notes" class="flex items-start gap-2">
                        ...
                        <span class="text-muted-2 text-xs leading-relaxed whitespace-pre-wrap">{{ item.arc_notes }}</span>
                    </div>
                </div>
            </div>
            ...
            <div v-if="item.notes" class="panel">
                ...
                <p class="text-muted-2 text-xs leading-relaxed whitespace-pre-wrap">{{ item.notes }}</p>
            </div>
            ...
            <div v-if="item.children && item.children.length" class="space-y-2">
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        <span v-if="child.pov_character" class="text-muted-3 text-xs font-mono ml-auto">
                            {{ child.pov_character.name }}
                        </span>
                        <span v-if="child.word_count" class="text-muted-3 text-xs font-mono ml-auto">
            ...
            <div class="panel">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-start gap-2">
                        ...
                        <span class="text-muted-2 text-xs">{{ formatLabel(item.visibility) }}</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <span class="field-label">Classification</span>
                        <span class="text-muted-2 text-xs">{{ formatLabel(item.content_classification) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end pt-2 border-t border-border">
                <button @click="destroy" class="btn-danger">Delete Item</button>
            </div>
...
const destroy = () => {
    if (!confirm(`Delete "${props.item.title}"? This cannot be undone.`)) return
...
.stage-badge--sm { padding: 1px 7px; font-size: 9px; }
...
.stat-value { font-size: 14px; color: var(--text-primary); font-weight: 300; }
...
.add-child-btn {
    display: block; padding: 8px 14px; margin-top: 4px;
