    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h1 class="text-primary text-2xl font-light tracking-wide">{{ title }}</h1>
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap mb-1">
                            <span class="text-primary text-sm font-light leading-snug">{{ item.title }}</span>
                            <span
                        ...
                        <p v-if="item.subtitle" class="text-muted-3 text-xs leading-relaxed mb-2">
                            {{ item.subtitle }}
                        </p>
        ...
        <div v-else class="empty-state">
            <p class="text-muted-3 text-xs font-mono uppercase tracking-widest mb-2">{{ emptyTitle }}</p>
            <Link v-if="emptyCtaHref" :href="emptyCtaHref" class="text-cyan text-xs font-mono hover:underline">
                {{ emptyCtaLabel }}
            </Link>
        </div>
    </AuthenticatedLayout>
